...
interface DialogContentProps
  ...
  /**
   * Whether to auto-focus the first focusable element (input/textarea/select) when dialog opens.
   * Defaults to true for better mobile keyboard behavior.
   */
...
const DialogContent = React.forwardRef<
  ...
  DialogContentProps
>(({ className, children, autoFocusFirst = true, onOpenAutoFocus, ...props }, ref) => {
  const handleOpenAutoFocus = React.useCallback(
    (event: Event) => {
      ...
      // Auto-focus first input/textarea/select for better mobile keyboard behavior
      if (autoFocusFirst) {
        event.preventDefault()
        // Small delay to ensure content is rendered
        setTimeout(() => {
          const content = (event.target as HTMLElement)
          const focusableElement = content?.querySelector<HTMLElement>(
    ...
    [autoFocusFirst, onOpenAutoFocus]
  )
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "text-lg font-semibold leading-none tracking-tight text-hb-cognac",
      className
    )}
    {...props}
  />
